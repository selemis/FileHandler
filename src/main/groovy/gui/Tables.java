...
public class Tables {
    ...
    public static void setTableFont(JTable table, String fontName, int size, boolean boldHeader) {
        if (table == null || fontName == null || fontName.trim().length() == 0)
            return;
    ...
    public static void changeColumnWidth(JTable table, int columnNo, int width) {
        if (table == null || columnNo < 0 || columnNo >= table.getColumnModel().getColumnCount())
            return;
    ...
    public static void presetColumn(JTable table, String title, int width) {
        ...
        for (int i = 0; i < tableData.size(); i++)
            if (tableData.get(i).table == table)
                col++;
    ...
    private static void presetColumn(JTable table, int columnNo, boolean setTitle, String title, boolean setWidth,
            int width) {
        if (table == null || columnNo < 0)
            return;
        int at = -1;
        for (int i = 0; i < tableData.size(); i++) {
    ...
    private static int getColumnCountForTable(JTable table) {
        ...
        for (int i = tableData.size() - 1; i >= 0; i--) {
            TableData data = tableData.get(i);
            if (data.table == null || data.table != table)
                continue;
            if (data.column + 1 > columnCount)
                columnCount = data.column + 1;
        }
        return columnCount;
    }
    ...
    public static void finalizeColumns(JTable table) {
        ...
        table.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS); // default
                                                                 // behaviour
    }
    ...
    public static int jumpSelectTableRow(JTable table, int columnNo, char c) {
        ...
        for (int i = row + 1; i <= upTo; i++) {
            Object value = table.getModel().getValueAt(i, columnNo);
            ...
            public boolean isCellEditable(int rowIndex, int columnIndex) {
                ...
                if (controller != null) {
                    ...
                    for (int i = 0; i < controller.nonEditable.size(); i++)
                        if (controller.nonEditable.get(i).equals(point))
                            return false;
                    for (int i = 0; i < controller.editable.size(); i++)
                        if (controller.editable.get(i).equals(point))
                            return true;
                }
                for (int i = 0; i < columns.length; i++) {
                    if (columnIndex == columns[i])
                        return isEditable;
                }
                return !isEditable;
            }
    ...
    private static EditController getEditController(JTable table) {
        ...
        for (int i = 0; i < editControllers.size(); i++)
            if (editControllers.get(i).table == table)
                return editControllers.get(i);
        return null;
    }
    ...
    public static void setCellEditable(JTable table, final int row, final int column, final boolean isEditable) {
        ...
        for (int i = controller.nonEditable.size() - 1; i >= 0; i--)
            if (controller.nonEditable.get(i).equals(point))
                controller.nonEditable.remove(i);
        for (int i = controller.editable.size() - 1; i >= 0; i--)
            if (controller.editable.get(i).equals(point))
                controller.editable.remove(i);
        if (isEditable)
     ...
     * @return
     */
    public static Object[] getTableRow(JTable table, int row) {
        if (table == null || row < -1 || row > table.getRowCount() - 1 || table.getColumnCount() == 0)
            return new Object[0];
        if (row == -1)
            return getTableHeaders(table);
        Object o[] = new Object[table.getColumnCount()];
        for (int i = 0; i < table.getColumnCount(); i++)
            o[i] = table.getModel().getValueAt(row, i);
        return o;
    }
     ...
     * @return
     */
    public static Object getTableCell(JTable table, MouseEvent mouseEvent) {
        Point coords = getTableCellCoords(table, mouseEvent);
        if (coords == null)
            return null;
        return getTableCell(table, coords.y, coords.x);
    }

    public static Object getTableCell(JTable table, int row, int column) {
    ...
    public static void setTableCell(JTable table, int row, int column, Object value) {
        if (row < 0 || row > table.getRowCount() - 1 || column < 0 || column > table.getColumnCount() - 1)
            return;
     ...
     * @param mouseEvent
     *            the mouse event
     * @return a Point with the (column, row) coordinates. If the table is null, the mouse event is null, or the mouse event did
     *         not take place within the table, the returned value is null
     */
    public static Point getTableCellCoords(JTable table, MouseEvent mouseEvent) {
        if (table == null || mouseEvent == null)
            return null;
        Point at = new Point(mouseEvent.getX(), mouseEvent.getY());
        return getTableCellCoords(table, at);
    }
     ...
     * @return a Point with the (column, row) coordinates. If the table is null, the coordinates point is null, or the
     *         coordinates point is not inside the table, the returned value is null
     */
    public static Point getTableCellCoords(JTable table, Point coordinates) {
        if (table == null || coordinates == null)
            return null;
     ...
     */
    public static List<Integer> getSelectedRows(JTable table) {
    ...
    /**
     * Moves all currently selected rows up or down in the table rows positions by amount defined by user.
     *
     * @param table
     * @param moveAmount
     *            use a positive number to move the selected rows towards the end of the table, or a negative number to move the
     *            selected rows towards the beginning of the table. If the rows reach the boundaries of the table (i.e.
     *            <code>moveAmount</code> is too large), then the rows start packing at the beginning or the end of the table
     *            without generating an Exception.
     */
    public static void moveSelectedRows(JTable table, int moveAmount) {
        if (table.getSelectedRowCount() == 0)
            return;
        List<Integer> selectedRows = getSelectedRows(table);
        int newFirstRow = -1;
        table.clearSelection();
        for (int i = 0; i < selectedRows.size(); i++) {
            int targetRow = -1;
            if (moveAmount < 0)
                targetRow = moveSelectedRow(table, selectedRows, i, moveAmount);
            else
                targetRow = moveSelectedRow(table, selectedRows, selectedRows.size() - i - 1, moveAmount);
            if (newFirstRow == -1)
                newFirstRow = targetRow;
            else if (targetRow != -1)
                newFirstRow = Math.min(newFirstRow, targetRow);
        }
        if (newFirstRow != -1)
            scrollToRow(table, newFirstRow);
    }

    private static int moveSelectedRow(JTable table, List<Integer> selectedRows, int selectedRowIndex, int shift) {
        int selectedRow = selectedRows.get(selectedRowIndex);
        int targetRow = selectedRow + shift;
        if (targetRow != selectedRow) {
            targetRow = Math.max(selectedRowIndex, targetRow);
            targetRow = Math.min(targetRow, table.getRowCount() - (selectedRows.size() - selectedRowIndex));
            DefaultTableModel model = (DefaultTableModel)table.getModel();
            model.moveRow(selectedRow, selectedRow, targetRow);
        }
        selectTableRow(table, targetRow, true);
        return targetRow;
    }
